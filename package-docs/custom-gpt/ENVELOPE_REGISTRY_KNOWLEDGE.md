# @omnituum/envelope-registry — Knowledge

**Date:** 2026-04-23
**Module:** envelope-registry
**Registry entry:** `governance/module-system/MODULE_INDEX.json` → `modules[]` where `module == 'envelope-registry'`
**Canonical-4 root:** `libs/Omnituum/envelope-registry/package-docs/`
**Prefix:** `**ER-##**` — see DOC_STANDARD § Module-Scoped ID Grammar.

---

## Deployment / Runtime State

Authoritative source: `libs/Omnituum/envelope-registry/package-docs/ENVELOPE_REGISTRY_OVERVIEW.md` § Where It Sits.
...
Authoritative source: filesystem state at `libs/Omnituum/envelope-registry/`. Capture via `tree libs/Omnituum/envelope-registry -L 3` (or `ls -R`) and paste under this section at refresh time. Do not duplicate inline here — re-render on refresh.

## Shared Utilities

Authoritative source: `libs/Omnituum/envelope-registry/package.json` (or `pyproject.toml` / `Cargo.toml` per language) dependencies + `libs/Omnituum/envelope-registry/package-docs/ENVELOPE_REGISTRY_OVERVIEW.md` § Where It Sits > Depends on.

## Issues / Tasks

**Authoritative file:** `libs/Omnituum/envelope-registry/package-docs/GAPS_AND_TASKS.md`.
...
- Prefix grammar: `**ER-##**` where `##` is zero-padded as required; optional lowercase suffix allowed per § Module-Scoped ID Grammar.

## Cross-Dependencies

Authoritative: `governance/module-system/MODULE_INDEX.json` entry for `envelope-registry` — fields `blocked_by` (this module needs these first) + `enables` (these modules need this module). Re-read on refresh.

## Valid Issue IDs

Shape: `**ER-##**` (optionally with lowercase suffix: `**ER-##a**`).
